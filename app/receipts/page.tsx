...
export default function ReceiptsPage() {
    ...
    const handlePDF = async (receiptId: string) => {
        const success = await generateReceiptPDF(receiptId, token || '');
        if (success) {
    ...
    return (
        <div className="p-6">
            <div className="flex justify-between items-center mb-6">
                <h1 className="text-2xl font-bold text-gray-900">Phiếu nhập kho ({receipts.length})</h1>
                <button
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                        {receipts.map((receipt) => (
                            <tr key={receipt.id} className="hover:bg-gray-50">
                                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">{receipt.code}</td>
                                <td className="px-6 py-4 whitespace-nowrap text-sm">{receipt.supplier}</td>
                                <td className="px-6 py-4 whitespace-nowrap text-sm">{new Date(receipt.date).toLocaleDateString('vi-VN')}</td>
                                <td className="px-6 py-4 whitespace-nowrap text-sm font-semibold text-green-600">{receipt.totalAmount.toLocaleString()} đ</td>
                                <td className="px-6 py-4 whitespace-nowrap text-sm">
                                    <span className={`px-2 py-1 rounded text-xs ${receipt.status === 'COMPLETED' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}`}>
                                        {receipt.status}
                                    </span>
                                </td>
                                <td className="px-6 py-4 whitespace-nowrap text-sm">
